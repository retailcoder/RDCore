...
|2026-09-24|1.5|§3.5.4/§3.5.5 — procedure invocation (`IProcedureInvoker`/`RuntimeProcedureInvoker`, `Call`/bare-call/bare-`Sub`/`Function`/`Property Get`), `ByRef` parameter binding (`CallStackFrame.PushByRef`, `ISymbolResolver.TryGetAddress`), `Function`/`Property Get` return values (`ICallStackFrame.ReturnValue`, the function result variable), and hoisted `Dim`/`Static` locals (`VBProcedureMemberSymbol.Locals`/`VBReturningMemberSymbol.Locals` riding on the procedure symbol like `Parameters`, `RuntimeProcedureInvoker.HoistLocals`, `ISymbolResolver.TryAllocate` for a `Static` local's own module-extent storage)|
| | | |
